...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, IdePanelComponent],
  template: `
    <!-- IDE PANEL REPLACE LOGIC -->
    <div *ngIf="flowService.activeModules().ide" class="h-full w-full">
        <app-ide-panel class="h-full w-full block"></app-ide-panel>
    </div>

    <!-- STANDARD SIDEBAR LOGIC -->
    <div 
      *ngIf="!flowService.activeModules().ide"
      ...
      [class.border-r]="flowService.theme() === 'cold' && !flowService.extraGlassMode()"
    ...
    >
      ...
      @if (!flowService.zenMode()) {
        <div class="flex items-center justify-between mb-10 px-1 animate-in fade-in duration-500">
           <!-- Click to Exit / Home -->
           <button (click)="flowService.exitSession()" class="flex items-center gap-3 hover:opacity-80 transition-opacity">
              ...
              <div class="flex flex-col text-left">
                ...
                <span class="text-[10px] text-[#8E918F]">Workspace</span>
              </div>
           </button>
      ...
      <div class="mt-4 flex-shrink-0 relative z-20 flex flex-col gap-3">
        ...
        <div class="flex gap-2">
            <!-- IDE Toggle Button -->
            <button
                (click)="flowService.updateModule('ide', !flowService.activeModules().ide)"
                ...
                [class.glass-panel-ultra]="flowService.extraGlassMode()"
                [class.bg-[#D0BCFF]]="flowService.activeModules().ide && !flowService.extraGlassMode()"
                [class.text-[#381E72]]="flowService.activeModules().ide && !flowService.extraGlassMode()"
                [class.hover:bg-[#EADDFF]]="flowService.activeModules().ide && !flowService.extraGlassMode()"
                title="Toggle IDE"
            >
